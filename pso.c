...
static double vmax_x=0;
static double vmax_y=0;
...
void init_particles(particle *dron, swarm *roj, int ilosc, int zakres_x, int zakres_y, int **matrix){
    for(int i=0;i<ilosc;i++){
        ...
        dron[i].trust = 0.9;
        dron[i].random_val = stochastic_elem();
        dron[i].weight = 0.5;
    }
    roj->random_val = stochastic_elem();
    roj->trust = 1.0;
}
...
coordinates max_swarm_signal(int **matrix, particle dron, swarm roj){
    ...
    if(matrix[dron.best_position.x][dron.best_position.y]>=matrix[roj.best_position.x][roj.best_position.y]){
        ...
        new_swarm_best.x=(int)dron.best_position.y;
    }
...
position new_particle_velocity(particle dron, swarm roj){
    ...
    dron.random_val = stochastic_elem();
    roj.random_val = stochastic_elem();
    new_velocity.x = dron.weight*dron.velocity.x + dron.trust*dron.random_val * (dron.best_position.x - dron.current_position.x) + roj.trust*dron.random_val * (roj.best_position.x - dron.current_position.x);
    new_velocity.y = dron.weight*dron.velocity.y + dron.trust*dron.random_val * (dron.best_position.y - dron.current_position.y) + roj.trust*dron.random_val * (roj.best_position.y - dron.current_position.y);
